...
class csv2geo {
    ...
    gpx = (csvFile, options) => new Promise((resolve, reject) => {
        ...
        <gpx version="1.1" creator="csv2geo" xmlns="http://www.topografix.com/GPX/1/1">
        <metadata>
            <desc>Generated by csv2geo</desc>
        </metadata>`;
        let gpxFooter = `</gpx>`;
        let gpxBody = "";
        ...
        fs.createReadStream(csvFile)
            ...
            .on("end", function () {
                ...
                if ((lonIndex == undefined || latIndex == undefined) || (lonIndex == -1 || latIndex == -1)) {
                    reject("Error while finding Lon-lat Values. Please use options parameter. doc:https://github.com/trgisdev/csv2geo");
                    return;
                }
                ...
                for (let i = 0; i < csvArr.length; i++) {
                    ...
                    let wpt = `<wpt lat="${lat}" lon="${lon}">
                        <desc>${JSON.stringify(currentRow)}</desc>
                    </wpt>`;
                    gpxBody += wpt;
                }
                ...
                if (saveFilePath != null) {
                    fs.writeFile(saveFilePath, gpxContent, err => {
                        if (err) {
                            reject("Error while writing GPX file. doc:https://github.com/trgisdev/csv2geo ", err);
                        }
                    });
                }
            ...
            .on("error", function (error) {
                reject("Error while reading CSV file. doc:https://github.com/trgisdev/csv2geo ", error);
            });
        return;
    })
